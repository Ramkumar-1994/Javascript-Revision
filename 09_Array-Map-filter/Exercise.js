...
let highRateMovies=movies.filter((movie)=>
{
  return movie.imdbRating>8.5
})
console.log(highRateMovies)

console.log(movies.length)//21

// - Filter the movies to create an array of movies starring Leonardo DiCaprio.

let starMovie=movies.filter((movie)=>
{
...
let chrisBaleMovies=movies.filter((movie)=>
{
  return movie.actors.includes('Christian Bale')
}).map((movie)=>
{
  return movie.title
})

console.log(chrisBaleMovies)
// - Chain operations to find all drama movies featuring Tim Robbins.

let timRobbinsMovies=movies.filter((movie)=>
  {
    return movie.actors.includes('Tim Robbins')
  }).map((movie)=>
  {
    return movie.title
  })
